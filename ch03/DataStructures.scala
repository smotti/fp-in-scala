...
/**
 * There's also a cons opertator "::" that can used for pattern matching:
 ...
 *
 * But doesn't seem to work with our custom trait.
 */
...
object List {
  ...
  */
  ...
  // More elegant solution for Exercise 3.4
  @annotation.tailrec
  def drop[A](as: List[A], n: Int): List[A] = n match {
    case 0 => as
    case i if i < 0 => sys.error("n must be >= 0")
